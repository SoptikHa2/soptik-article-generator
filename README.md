...
```sh
# We hav syntax highlighting now!
for file in *".md" {
	do-magic "$file"
}
```
...
- anything in `assets` folder is copied as-is into html (so one can easily add images and videos there)
- If source file contains `--no-index`, it won't be included in index list and tags list.
